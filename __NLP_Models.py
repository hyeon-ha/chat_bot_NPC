...
class LSTM_model() :
    def __init__(self, vocab_size, X_train, X_test, Y_train, Y_test, korean_package, embedding_dim = 50, hidden_units = 128):

        self.model = Sequential()
        self.model.add(Embedding(vocab_size, embedding_dim))  ##한 문장를 100차원 벡터값으로 표현 그 shape ( -1,1)
        self.model.add(LSTM(hidden_units))  ### padded 모양 맞춘거
        # self.model.add(Dense(3, activation='softmax'))
        self.model.add(Dense(3, activation='softmax'))
        ###1차 시도 차원을 줄여보기로

        model_save_path = './save_as_try'
        es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=4) ##변화가 없음 멈추겠다
